...
# Filter columns
columns_to_plot = [col for col in data.select_dtypes(include=['float64', 'int64']).columns if ("MS" and 'IS') not in col and col != "RelativeTime_Hours"]
...
ax1.set_ylabel('Normalized Peak Area')  # Primary y-axis label
...
plt.title('Normalized Peak Area vs Relative Time (hours)')
plt.show()
